...
*Crawl → discover → audit. **27 plugins**, 6 report formats, polite defaults, content-verified findings.*
...
| **Passive-first design** | 8 of 27 plugins are passive (no probes sent) — `headers`, `cookies`, `cors`, `ssl_tls`, `tech_fingerprint`, `security_txt`, `robots_sitemap`, `jwt_audit` | Site owners can audit configuration exposure without sending a single probe |
...
**27 plugins** — each content-verified to keep the false-positive rate low.
...
|---------|--------|---------|
| 🟢 **WebScan** | ★★★★★ | **Fastest (7.3s)**, most findings (28), **zero false positives**, 350K CVE real-time, 27 plugins with content verification, free MIT |
